...
% plot amplitude
plot(fig5.dft(1).dist, 20*log10(fig5.dft(1).Er.amp), 'b', 'lineWidth', 1); 
hold on;
plot(fig5.dft(2).dist, 20*log10(fig5.dft(2).Er.amp), 'r', 'lineWidth', 1);
...
set(gca,'defaultAxesFontSize',16);
% plot phase
plot(fig5.dft(1).dist, pha1, 'b', 'lineWidth', 1); 
hold on;
plot(fig5.dft(2).dist, pha2, 'r', 'lineWidth', 1);
% plot receiver locations
x = [recCO recCO]; y = [-1e3 1e3];
plot(x, y, 'k--', 'lineWidth', 1);
x = [recUT recUT]; y = [-1e3 1e3];
